...
Te time series for the AOV shows a spike in sells for the Beginning and **end** of the month. For the rest of the month, the spike is somewhat consistent.
...
The time series for the AOV for the data without outlier shows a spike in AOV for the middle of the month. This is opposite of the first time series (one with outliers). In Beginning and **end** of the month, their is a decline ni AOV.
...
-   What Should matter to the store is if they are able to retain customers. Given the time period of the data set, it would be difficult to calculate customer retention.
...
retention <- function(data) {
  ...
  retention_plot <- cohort_days%>%
  filter( time > 0, value > 0) %>%
  ggplot(aes(time, value, color = cohort)) +
  geom_line(size = 1) +
  geom_point(size = 1) +
  facet_wrap(~cohort) +
  xlab('Days in March') +
  ylab('z% Retention Rate ') +
  ggtitle('Retention rate over Time')
...
**Rention Rate for shop with the highest uni_cost**

The cost of a sneaker at **shop 78** is : **25,725 dollars.**

```{r}
max_shop <-data |>
  filter(unit_cost == min(unit_cost))

max_shop
```

**Do they have an high retention rate?**
